...
export async function getHosts(
  ...
  if (import.meta.env.DEV) {
    console.log("[getHosts] params:", params);
    console.log("[getHosts] url:", url);
    console.log("[getHosts] token present:", !!authToken);
  }
...
// 비인증 환경(제보 페이지 등)에서 사용하는 공개 조회용 API
export async function getAllHostsPublic(): Promise<HostResponse[]> {
  const url = `${API_BASE}/api/v1/hosts?size=1000`;
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(
      `Failed to fetch all hosts (public): ${res.status} ${text}`
    );
  }
  const response = (await res.json()) as HostListResponse;
  return response.content;
